...
def _refresh_data():
    ...
    I_MRP = find_col(inv.columns, "MRP")
...
<div class="wrap">
  ...
  <div class="pm-note" id="pmNote">Pick a SKU — MRP and Product Cost load from the All Product sheet (columns N &amp; M). You can still edit anything.</div>
...
function pmSetMode(m){
  ...
  if (m==='old'){
    document.getElementById('pmNote').innerHTML = 'Pick a SKU — MRP and Product Cost load from the All Product sheet (columns N &amp; M). You can still edit anything.';
...
function pmLoadCatalog(){
  ...
  fetch('/api/sku-costs', {headers:{'ngrok-skip-browser-warning':'true'}})
    .then(r => r.ok ? r.json() : Promise.reject(new Error('HTTP ' + r.status)))
    .then(d => {
      if (d.error){ st.textContent = d.error; return; }
      PM_CATALOG = d.rows || [];
      pmLoaded = true;
      st.textContent = PM_CATALOG.length.toLocaleString('en-IN') + ' SKUs loaded — start typing a SKU.';
    })
    .catch(err => { st.innerHTML = 'Could not load SKUs. Switch to <b>New Product</b> to enter manually.'; });
}
function pmSkuType(){
  ...
  list.innerHTML = matches.length
    ? matches.map(c => {
        ...
        return `<div class="pm-skuopt" onclick="pmPickSku('${c.sku.replace(/'/g,"\\\\'")}')"><span>${im}${escHtml(c.sku)}</span><span class="mrp">${c.mrp?pmRupee(c.mrp):'—'}</span></div>`;
      }).join('')
...
def _build_sku_costs():
    ...
    # Compiled data se sku, image, final_qty, net_revenue
    img_map = {}; qty_map = {}; rev_map = {}
    order = []
    try:
        comp = get_data()[0]
        for it in comp:
            ...
            if not sk:
                continue
            ku = sk.upper()
            if ku not in img_map:
                order.append(sk)
            img_map[ku] = it.get("image_url", "") or ""
            qty_map[ku] = int(it.get("final_qty") or 0)
            rev_map[ku] = float(it.get("total_net_revenue") or 0)
    except Exception:
        pass

    # MRP (N=index13) aur Cost (M=index12) — SEEDHE All Product sheet se.
    # Cost = "Cost" column (M), MRP = "MRP" column (N). Naam pehle, fir position.
    mrp_map = {}; cost_map = {}
    try:
        ...
        def _at(i): return cols[i] if len(cols) > i else None
        C_SKU  = _at(0)
        C_COST = find_col(df.columns, "Cost", "cost", "hi cost", "product cost") or _at(12)  # M
        C_MRP  = find_col(df.columns, "MRP", "mrp", "m.r.p") or _at(13)                       # N
        ...
        for _, r in df.iterrows():
            ...
            if not sku:
                continue
            ku = sku.strip().upper()
            if ku not in cost_map:
                cost_map[ku] = to_num(r.get(C_COST, 0)) if C_COST else 0
            if ku not in mrp_map:
                mrp_map[ku] = to_num(r.get(C_MRP, 0)) if C_MRP else 0
            if ku not in img_map or not img_map.get(ku):
                img_map[ku] = clean(r.get(C_IMG2, "")) if C_IMG2 else ""
            if ku not in qty_map:   # SKU jo compiled (COSSA) me nahi tha — fir bhi list me aaye
                order.append(sku)
    except Exception:
        pass
    ...
    for sku in order:
        ...
        out.append({
            "sku": sku,
            "image_url": img_map.get(ku, ""),
            "mrp":  mrp_map.get(ku, 0),
            "cost": cost_map.get(ku, 0),
            "final_qty":   qty_map.get(ku, 0),
            "net_revenue": rev_map.get(ku, 0),
        })
...
def search():
    try:
        if not AI_READY:
            return jsonify({"error": "AI model not loaded (upload sku_finder.pkl and set GEMINI_KEY)"}), 500
...
def _warmup_and_refresh_loop():
    ...
    try:
        ...
        # vision init bhi memory leta hai — data ready hone ke baad, alag try me
        try:
            init_vision()
            _vision_self_test()
        except Exception as e:
