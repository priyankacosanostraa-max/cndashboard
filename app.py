...
# ════════════════════════════════════════════════════════════════
#  🏷️ COSANOSTRAA.COM SKU → PRODUCT NAME
#  cosanostraa.com ek Shopify store hai — uska public /products.json
#  endpoint (koi API key nahi chahiye) saare products + variant SKUs
#  deta hai. Isse SKU -> asli product naam ka map banta hai.
#  Jis SKU ka match cosanostraa.com par mil jaye uska ORIGINAL NAME
#  dikhega; jo SKU wahan nahi hai uska SKU code jaisa hai waisa hi
#  (bina chhede) dikhega.
# ════════════════════════════════════════════════════════════════
...
def _cn_base_code(s):
    """'BH-1382-CELB' -> 'BH-1382' jaisa core code nikalta hai taaki
    suffix (-CELB, -SET, wagera) waale variants bhi match ho jaayein."""
    ns = _cn_normalize(s)
    m = re.match(r"^([A-Z]{1,5}-?\d{2,6})", ns)
    if not m:
        return ""
    b = m.group(1)
    if "-" not in b:
        mm = re.match(r"^([A-Za-z]+)(\d.*)$", b)
        if mm:
...
def _cn_fetch_all_products():
    ...
    while page <= 60:   # safety cap (~15,000 products)
        ...
        try:
            ...
            print("cosanostraa.com catalog fetch failed:", str(e)[:120])
            break
        if not products:
            break
        out.extend(products)
...
def cn_catalog_is_stale():
    return (time.time() - CN_CATALOG["ts"]) > CN_REFRESH_INTERVAL or CN_CATALOG["count"] == 0

def cn_build_catalog(force=False):
    """SKU -> product name map (re)build karta hai. Sirf background
    thread se call hota hai isliye race condition ka practical risk
    nahi (worst case ek refresh cycle purana data)."""
    if not force and not cn_catalog_is_stale():
        return
    try:
        ...
        print("cosanostraa.com catalog build failed:", str(e)[:160])
        CN_CATALOG["ts"] = time.time()  # avoid hammering on repeated failures

def cn_display_name(sku):
    """SKU ke liye cosanostraa.com ka original product naam. Match nahi
    mila to None (SKU code as-is rehne dena caller ki zimmedari)."""
    if not sku:
        return None
    ns = _cn_normalize(sku)
    name = CN_CATALOG["exact"].get(ns)
    if name:
        return name
    b = _cn_base_code(ns)
    if b:
...
def cn_sku_label(sku):
    ...
    SKU code bilkul waisa hi jaisa hai."""
...
/* Rakhi tab: compact, wrapped and professional tables */
#vRakhi .ro-table-wrap{max-height:68vh;border-radius:14px;box-shadow:0 8px 24px rgba(15,23,42,.07)}
#vRakhi table.ro{table-layout:auto;min-width:100%;font-size:11px}
#vRakhi table.ro thead th{position:sticky;top:0;z-index:3;white-space:normal;line-height:1.25;min-width:72px;max-width:180px;padding:11px 10px;vertical-align:middle}
...
/* Planning Intelligence tabs */
.ops-page .ops-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;flex-wrap:wrap;margin-bottom:12px}
.ops-page .ops-title{font-family:'Cormorant Garamond',serif;font-size:31px;line-height:1.05;font-weight:800;color:var(--cn-dark);letter-spacing:.3px}
.ops-page .ops-sub{margin-top:6px;font-size:10px;line-height:1.55;color:#74694e;font-weight:700;max-width:930px}
.ops-page .ops-actions{display:flex;gap:8px;flex-wrap:wrap}
.ops-page .ops-filters{display:grid;grid-template-columns:repeat(6,minmax(145px,1fr));gap:12px;background:#fff;border:1px solid var(--cn-line);border-radius:17px;padding:15px;margin:10px 0 16px;box-shadow:0 8px 24px rgba(15,23,42,.04)}
.ops-page .ops-filters .fc{min-width:0}
.ops-page .ops-kpis{display:grid;grid-template-columns:repeat(4,minmax(155px,1fr));gap:12px;margin:0 0 16px}
.ops-page .ops-kpi{background:#fff;border:1px solid var(--cn-line);border-radius:16px;padding:17px 18px;box-shadow:0 8px 24px rgba(15,23,42,.05);min-height:92px}
.ops-page .ops-kpi-label{font-size:9px;letter-spacing:1.9px;text-transform:uppercase;color:#8c7a42;font-weight:850;margin-bottom:8px}
.ops-page .ops-kpi-value{font-family:'Cormorant Garamond',serif;font-size:29px;line-height:1;color:var(--cn-gold);font-weight:800}
.ops-page .ops-kpi-sub{font-size:10px;color:#64748b;font-weight:650;margin-top:7px;line-height:1.35}
.ops-page .ops-section{margin-top:25px}
.ops-page .ops-section-head{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:9px}
.ops-page .ops-section-title{font-size:18px;color:var(--cn-dark);font-weight:850}
.ops-page .ops-note{font-size:10px;line-height:1.55;color:#74694e;font-weight:650;margin-top:8px}
.ops-page .ops-table-wrap{overflow:auto;max-height:67vh;border:1px solid var(--cn-line);border-radius:16px;background:#fff;box-shadow:0 10px 28px rgba(15,23,42,.05)}
.ops-page table.ops-table{width:100%;border-collapse:collapse;min-width:1180px;table-layout:auto}
.ops-page table.ops-table th{position:sticky;top:0;z-index:3;background:var(--cn-ivory);color:var(--cn-gold);font-size:9px;letter-spacing:1.4px;text-transform:uppercase;padding:11px 10px;border-bottom:1px solid var(--cn-line);white-space:nowrap;text-align:left}
.ops-page table.ops-table td{padding:10px;border-bottom:1px solid #eee9df;vertical-align:middle;font-size:11px;color:#334155;font-weight:700;line-height:1.4}
.ops-page table.ops-table tbody tr:hover td{background:#fffdf8}
.ops-page .ops-photo,.ops-page .ops-photo-ph{width:76px;height:76px;min-width:76px;border:1px solid #e5dcc8;border-radius:11px;background:#fff;padding:4px;box-sizing:border-box;display:flex;align-items:center;justify-content:center;margin:auto}
.ops-page .ops-photo{object-fit:contain;object-position:center}
.ops-page .ops-photo-ph{font-size:25px;color:#c0ad76}
.ops-page .ops-badge{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;padding:5px 9px;font-size:9px;font-weight:850;letter-spacing:.25px;white-space:nowrap;border:1px solid transparent}
.ops-page .ops-badge-red{background:#fee2e2;color:#991b1b;border-color:#fecaca}
.ops-page .ops-badge-orange{background:#ffedd5;color:#9a3412;border-color:#fed7aa}
.ops-page .ops-badge-gold{background:#fef3c7;color:#92400e;border-color:#fde68a}
.ops-page .ops-badge-blue{background:#e0f2fe;color:#075985;border-color:#bae6fd}
.ops-page .ops-badge-green{background:#dcfce7;color:#166534;border-color:#bbf7d0}
.ops-page .ops-badge-gray{background:#f1f5f9;color:#475569;border-color:#e2e8f0}
.ops-page .ops-list{white-space:normal;overflow-wrap:anywhere;min-width:220px;max-width:430px}
.ops-page .ops-num{text-align:right;font-variant-numeric:tabular-nums;white-space:nowrap}
.ops-page .ops-empty{padding:36px 20px;text-align:center;color:#8c7a42;font-size:12px;font-weight:750}
.ops-page .ops-divider{height:1px;background:linear-gradient(90deg,transparent,#d8c998,transparent);margin:24px 0}
@media(max-width:1150px){.ops-page .ops-filters{grid-template-columns:repeat(3,minmax(145px,1fr))}}
@media(max-width:820px){.ops-page .ops-kpis{grid-template-columns:repeat(2,minmax(140px,1fr))}.ops-page .ops-filters{grid-template-columns:repeat(2,minmax(135px,1fr))}.ops-page .ops-photo,.ops-page .ops-photo-ph{width:68px;height:68px;min-width:68px}}
@media(max-width:520px){.ops-page .ops-kpis,.ops-page .ops-filters{grid-template-columns:1fr}}

.ops-page .opp-score{display:inline-flex;align-items:center;justify-content:center;min-width:58px;padding:7px 11px;border-radius:999px;font-size:12px;font-weight:950;letter-spacing:.3px;border:1px solid transparent}
.ops-page .opp-score-high{background:#dcfce7;color:#166534;border-color:#bbf7d0}
.ops-page .opp-score-mid{background:#e0f2fe;color:#075985;border-color:#bae6fd}
.ops-page .opp-score-watch{background:#fef3c7;color:#92400e;border-color:#fde68a}
.ops-page .opp-score-low{background:#fee2e2;color:#991b1b;border-color:#fecaca}
.ops-page .opp-growth-up{color:#15803d;font-weight:900}.ops-page .opp-growth-down{color:#b3261e;font-weight:900}.ops-page .opp-growth-flat{color:#64748b;font-weight:850}

.ops-page .anom-metric{font-weight:950;color:#b3261e;white-space:nowrap}
.ops-page .anom-detail{min-width:300px;max-width:520px;white-space:normal;line-height:1.55;color:#475569}
...
<div class="wrap">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:14px">
    <div><div class="insights-head" style="margin-bottom:8px">
        ...
        <button class="go-btn" style="width:auto;padding:6px 12px;font-size:.65rem;letter-spacing:1px;background:#2f6f3e" onclick="exportPayToday()">Export CSV</button>
      </div>
    ...
    <div><div class="insights-head" style="margin-bottom:8px">
        ...
        <button class="go-btn" style="width:auto;padding:6px 12px;font-size:.65rem;letter-spacing:1px;background:#2f6f3e" onclick="exportPayMonthEnd()">Export CSV</button>
      </div>
      <div id="payMeTable" class="ro-table-wrap" style="padding:0;overflow:auto;max-height:55vh"></div></div>
  </div>
  <div style="margin-bottom:18px">
    <div class="insights-head" style="margin-bottom:8px">
      <div class="insights-title" style="font-size:1rem">Payments Received (Latest → Oldest)</div>
      <button class="go-btn" style="width:auto;padding:6px 12px;font-size:.65rem;letter-spacing:1px;background:#2f6f3e" onclick="exportPaymentsLog()">Export CSV</button>
    </div>
    <div style="color:var(--cn-mid);font-size:.68rem;margin-bottom:6px">Respects Tag / Search Customer filters above. Sorted by payment/credit date, newest first.</div>
    <div id="payLogTable" class="ro-table-wrap" style="padding:0;overflow:auto;max-height:55vh"></div>
  </div>
  <div style="display:grid;grid-template-columns:420px 1fr;gap:18px;margin-bottom:18px;align-items:start">
    <div>
      <div class="insights-head" style="margin-bottom:6px">
        ...
        <button class="go-btn" style="width:auto;padding:5px 10px;font-size:.6rem;letter-spacing:1px;background:#2f6f3e" onclick="exportPayAging()">Export CSV</button>
      </div>
  ...
  <div style="margin-bottom:18px">
    <div class="insights-head" style="margin-bottom:6px">
      ...
      <button class="go-btn" style="width:auto;padding:5px 10px;font-size:.6rem;letter-spacing:1px;background:#2f6f3e" onclick="exportPayTagSummary()">Export CSV</button>
    </div>
  ...
  <div id="vRepeatPlanner" class="ops-page" style="display:none">
    <div class="ops-head">
      ...
      <div class="ops-actions">
        ...
        <button class="go-btn" style="width:auto;padding:10px 14px;letter-spacing:2px;background:#2f6f3e" onclick="exportRepeatPlanner()">Export CSV</button>
      </div>
    </div>
    ...
    <div class="ops-note">Inv WIP is taken from the current Inventory sheet. Window Sale is the total sold quantity inside the selected Sales Velocity period. This is a planning recommendation; MOQ, vendor capacity, cancellations and production lead-time variability should be reviewed before placing the order.</div>
  </div>
  ...
  <div id="vComboRisk" class="ops-page" style="display:none">
    <div class="ops-head">
      ...
      <div class="ops-actions">
        ...
        <button class="go-btn" style="width:auto;padding:10px 14px;letter-spacing:2px;background:#2f6f3e" onclick="exportComboRisk()">Export CSV</button>
      </div>
    </div>
    <div class="ops-filters">
      ...
      <div class="fc"><label class="fl">Risk Rows</label><select class="fs" id="crRiskOnly" onchange="renderComboRisk()"><option value="yes">Only Shortage Risk</option><option value="all">All Child SKUs</option></select></div>
  ...
  <div id="vOpportunity" class="ops-page" style="display:none">
    <div class="ops-head">
      ...
      <div class="ops-actions">
        ...
        <button class="go-btn" style="width:auto;padding:10px 14px;letter-spacing:2px;background:#2f6f3e" onclick="exportOpportunityScore()">Export CSV</button>
      </div>
    </div>
    <div class="ops-filters">
    ...
    <div class="ops-note">Scoring: Sales Growth = 40 points; Stock Availability = 30 points based on Inv Stock + Inv WIP versus the selected-window demand; Return Health = 30 points, reducing to zero at a 20% return rate. This is an opportunity-prioritisation signal, not a purchase-order approval.</div>
  </div>
  ...
  <div id="vSalesAnomaly" class="ops-page" style="display:none">
    <div class="ops-head">
      ...
      <div class="ops-actions">
        <button class="go-btn" style="width:auto;padding:10px 14px;letter-spacing:2px" onclick="loadSalesAnomalies(true)">Refresh</button>
        <button class="go-btn" style="width:auto;padding:10px 14px;letter-spacing:2px;background:#2f6f3e" onclick="exportSalesAnomalies()">Export CSV</button>
      </div>
    </div>
    <div class="ops-filters">
    ...
    <div class="ops-note">Detection basis: current selected window is compared with the immediately preceding equal window. Sale drop requires at least the selected baseline sale and a 50%+ decline; spike requires at least 2x growth with a meaningful unit increase; return increase requires a 10%+ current return rate and a material rise. Stock movement without sale uses the latest PPC-WIP receiving date as the stock-movement signal. Revenue checks are available to Admin because employee payloads intentionally hide revenue.</div>
  </div>
  ...
  <div id="vDemandPatterns" class="ops-page" style="display:none">
    <div class="ops-head">
      ...
      <div class="ops-actions">
        ...
        <button class="go-btn" style="width:auto;padding:10px 14px;letter-spacing:2px;background:#2f6f3e" onclick="exportDemandPatterns()">Export CSV</button>
      </div>
    </div>
    <div class="ops-filters">
  ...
  <div id="vOosLost" class="ops-page" style="display:none">
    <div class="ops-head">
      ...
      <div class="ops-actions">
        ...
        <button class="go-btn" style="width:auto;padding:10px 14px;letter-spacing:2px;background:#2f6f3e" onclick="exportOosLostSales()">Export CSV</button>
      </div>
    </div>
    <div class="ops-filters">
...
function _rkhBuildOverallSummary(){
  ...
  // Every curated Rakhi SKU, including SKUs with zero sales.
  const bySku = {};
  wlRows.forEach(r => {
  ...
  const skuRows = Array.from(RAKHI_WHITELIST_SKUS).map(rawSku => {
    ...
    else if (skuDrr >= 2) points.push('Strong run-rate — protect stock availability.');
...
let _rakhiPivotRows = [];
let _rakhiPivotTypes = [];
function _rkhBuildPivot(rows){
  const typeSet = new Set();
  const map = {};
  rows.forEach(r => {
    const key = String(r.sku || '').trim();
    if (!key) return;
    const t = String(r.type || '').trim() || 'Unspecified';
    typeSet.add(t);
    if (!map[key]) map[key] = {sku: r.sku, sku_name: r.sku_name, image_url: r.image_url, inv_stock: r.inv_stock || 0, inv_wip: r.inv_wip || 0, combo_details: r.combo_details || [], byType: {}, totalQty: 0, totalRev: 0};
    map[key].byType[t] = (map[key].byType[t] || 0) + (r.qty || 0);
    map[key].totalQty += (r.qty || 0);
    map[key].totalRev += (r.rev || 0);
  });
...
function renderRakhiPivot(){
  const host = document.getElementById('rakhiPivotContent');
  if (!host) return;
  if (!master || !master.length){
    host.innerHTML = '<div class="home-empty" style="padding:30px">Data still loading… please wait a moment.</div>';
    return;
  ...
  const rows = _rakhiFilteredRows || _rakhiRows || [];
  const {types, list} = _rkhBuildPivot(rows);
  _rakhiPivotRows = list; _rakhiPivotTypes = types;
  if (!list.length){
    host.innerHTML = '<div class="home-empty" style="padding:30px">No Rakhi orders match the selected filter.</div>';
    return;
  }
...
window.rkhOnTypeFilterChange = rkhOnTypeFilterChange;
...
function _rkhBuildSlowMovers(){
  const soldMap = {};
  // Type filter lga ho to "sold" sirf usi Type ke hisaab se count hota
  // hai (_rakhiFilteredRows), warna poore Rakhi season ka data (_rakhiRows).
  (_rakhiFilteredRows || _rakhiRows || []).forEach(r => {
...
function _opsDateDays(iso){
  ...
  const today = String(todayISO || _opsSupport.today || new Date().toISOString().slice(0,10)).slice(0,10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(today)) return null;
  const a = Date.UTC(...s.split('-').map((n,i)=>i===1?Number(n)-1:Number(n)));
  const b = Date.UTC(...today.split('-').map((n,i)=>i===1?Number(n)-1:Number(n)));
  if (!Number.isFinite(a) || !Number.isFinite(b)) return null;
  return Math.max(0, Math.floor((b-a)/86400000));
}
function _opsPhoto(url){
  const u = String(url || '').trim();
  if (!u || u.toLowerCase()==='nan') return '<div class="ops-photo-ph">💎</div>';
  return `<img class="ops-photo" src="${escHtml(u)}" loading="lazy" decoding="async" onerror="this.outerHTML='<div class=&quot;ops-photo-ph&quot;>💎</div>'">`;
}
function _opsGroup(it){ return _oosProductGroup(it); }
function _opsFillTaxon(id, rows, getter){
...
function _opsRiskBadge(key, label){
  const cls = key==='critical'||key==='red' ? 'red' : key==='high'||key==='orange' ? 'orange' : key==='medium'||key==='gold' ? 'gold' : key==='good'||key==='green' ? 'green' : key==='blue' ? 'blue' : 'gray';
  return `<span class="ops-badge ops-badge-${cls}">${escHtml(label)}</span>`;
}
function _opsKpi(label, value, sub){ return `<div class="ops-kpi"><div class="ops-kpi-label">${escHtml(label)}</div><div class="ops-kpi-value">${value}</div><div class="ops-kpi-sub">${escHtml(sub||'')}</div></div>`; }
function _opsQtyForWindow(it, days){
  if (days===7) return _opsNum(it.qty_7d);
  if (days===15) return _opsNum(it.qty_15d);
...
function _buildRepeatPlannerRows(){
  ...
  const lead = Math.max(1, _opsNum(document.getElementById('rpLeadDays')?.value || 45));
  ...
  _rpRows = (master||[]).map(it=>{
    ...
    const leadDemand = drr*lead;
    const safetyStock = drr*safetyDays;
    const raw = leadDemand+safetyStock-stock-wip;
...
function _repeatPlannerFiltered(){
  const rows = _rpRows.length ? _rpRows : _buildRepeatPlannerRows();
  const q=String(document.getElementById('rpSearch')?.value||'').trim().toLowerCase();
  const group=document.getElementById('rpGroup')?.value||'All';
...
function _buildComboRiskRows(){
  const childMap={};
  (master||[]).forEach(parent=>{
    const psku=_opsSkuKey(parent.sku); const details=Array.isArray(parent.combo_details)?parent.combo_details:[]; if(!psku||!details.length)return;
    const group=_opsGroup(parent); const seen=new Set();
    details.forEach(c=>{
      const csku=_opsSkuKey(c&&c.sku); if(!csku||seen.has(csku))return; seen.add(csku);
      const child=_masterSkuMap[csku]||c||{};
      if(!childMap[csku]) childMap[csku]={sku:csku,skuName:String(child.sku_name||c.sku_name||''),image:String(child.image_url||c.image_url||''),taxon:String(child.taxon||c.taxon||'General'),stock:Math.max(0,_opsNum(child.inv_stock??c.inv_stock)),wip:Math.max(0,_opsNum(child.inv_wip??c.inv_wip)),parents:[]};
      childMap[csku].parents.push({sku:psku,skuName:String(parent.sku_name||''),group,qty30:Math.max(0,_opsNum(parent.qty_1m)),drr:Math.max(0,_opsNum(parent.qty_1m))/30});
    });
  });
  const horizon=Math.max(1,_opsNum(document.getElementById('crHorizon')?.value||30));
  const includeWip=(document.getElementById('crIncludeWip')?.value||'no')==='yes';
  _crRows=Object.values(childMap).map(r=>{
    const demand=r.parents.reduce((s,p)=>s+p.drr*horizon,0); const support=r.stock+(includeWip?r.wip:0); const blocked=Math.max(0,Math.ceil(demand-support)); const cmbs=Array.from(new Set(r.parents.map(p=>p.sku))); const groups=new Set(r.parents.map(p=>p.group)); const group=groups.size===1?Array.from(groups)[0]:'Mixed';
    const risk=r.stock<=0&&demand>0?{key:'critical',label:'Child OOS'}:blocked>0?{key:'high',label:'Combo Production Blocked'}:support<demand*1.25?{key:'medium',label:'Low Buffer'}:{key:'good',label:'Covered'};
    return {...r,group,cmbs,demand,support,blocked,parent30:r.parents.reduce((s,p)=>s+p.qty30,0),risk};
  }).sort((a,b)=>b.blocked-a.blocked||b.demand-a.demand||b.cmbs.length-a.cmbs.length);
  _opsFillTaxon('crTaxon',_crRows,r=>r.taxon);
  return _crRows;
}
function _comboRiskFiltered(){
  const rows=_crRows.length?_crRows:_buildComboRiskRows(); const q=String(document.getElementById('crSearch')?.value||'').trim().toLowerCase(); const g=document.getElementById('crGroup')?.value||'All'; const tx=document.getElementById('crTaxon')?.value||'All'; const only=document.getElementById('crRiskOnly')?.value||'yes';
  return rows.filter(r=>(!q||`${r.sku} ${r.skuName} ${r.cmbs.join(' ')}`.toLowerCase().includes(q))&&(g==='All'||r.group===g||(g==='Rakhi'&&r.group==='Mixed'))&&(tx==='All'||r.taxon===tx)&&(only!=='yes'||r.blocked>0));
}
...
function renderComboRisk(){
  ...
  const impacted=new Set(); rows.forEach(r=>r.cmbs.forEach(x=>impacted.add(x))); const blocked=rows.reduce((s,r)=>s+r.blocked,0); const demand=rows.reduce((s,r)=>s+r.demand,0); const childOos=rows.filter(r=>r.stock<=0&&r.demand>0).length;
  if(sum)sum.innerHTML=_opsKpi('Risky Child SKUs',rows.filter(r=>r.blocked>0).length.toLocaleString('en-IN'),'Common and unique children')+_opsKpi('Impacted CMBs',impacted.size.toLocaleString('en-IN'),'Unique parent combos')+_opsKpi('Potential Blocked Qty',Math.round(blocked).toLocaleString('en-IN'),'Across selected horizon')+_opsKpi('Child OOS',childOos.toLocaleString('en-IN'),`Projected demand ${Math.ceil(demand).toLocaleString('en-IN')}`);
  const body=rows.map((r,i)=>`<tr><td class="ops-num">${i+1}</td><td>${_opsPhoto(r.image)}</td><td><button class="sku-link" onclick="openSkuDetails('${String(r.sku).replace(/'/g,"\\'")}')">${escHtml(skuLabel(r.sku,r.skuName))}</button></td><td>${escHtml(r.group)}</td><td>${escHtml(r.taxon)}</td><td class="ops-num">${r.cmbs.length.toLocaleString('en-IN')}</td><td class="ops-list">${r.cmbs.map(x=>`<button class="sku-link" onclick="openSkuDetails('${String(x).replace(/'/g,"\\'")}')">${escHtml(x)}</button>`).join(', ')}</td><td class="ops-num">${Math.round(r.stock).toLocaleString('en-IN')}</td><td class="ops-num">${Math.round(r.wip).toLocaleString('en-IN')}</td><td class="ops-num">${Math.round(r.parent30).toLocaleString('en-IN')}</td><td class="ops-num">${Math.ceil(r.demand).toLocaleString('en-IN')}</td><td class="ops-num" style="font-weight:900;color:${r.blocked>0?'#b3261e':'#15803d'}">${r.blocked.toLocaleString('en-IN')}</td><td>${_opsRiskBadge(r.risk.key,r.risk.label)}</td></tr>`).join('');
  host.innerHTML=`<table class="ops-table"><thead><tr><th>#</th><th>Photo</th><th>Child SKU</th><th>Combo Group</th><th>Child Taxon</th><th>No. of CMBs</th><th>Affected CMB SKUs</th><th>Child Stock</th><th>Child WIP</th><th>Parent 30D Sale</th><th>Horizon Child Demand</th><th>Potential Blocked Combo Qty</th><th>Risk</th></tr></thead><tbody>${body||'<tr><td colspan="13" class="ops-empty">No child-SKU shortages match the current filters.</td></tr>'}</tbody></table>`;
}
...
function _oppWindowStats(it, win){
  ...
  for(const e of (it.sales_entries||[])){
    const age=_oppEntryAgeDays(e.date); if(age===null) continue;
    const q=Math.max(0,_opsNum(e.qty)), r=Math.max(0,_opsNum(e.ret));
    if(age<win){ currentQty+=q; currentRet+=r; }
    else if(age<win*2){ previousQty+=q; previousRet+=r; }
  }
...
function _oppGrowthText(r){
  if(r.currentSale<=0 && r.previousSale<=0) return '<span class="opp-growth-flat">No sales</span>';
  if(r.previousSale<=0 && r.currentSale>0) return '<span class="opp-growth-up">New ↑</span>';
  const cls=r.growthPct>0?'opp-growth-up':r.growthPct<0?'opp-growth-down':'opp-growth-flat';
  const sign=r.growthPct>0?'+':'';
  return `<span class="${cls}">${sign}${r.growthPct.toFixed(1)}%</span>`;
}
function _oppActionFor(score, growthPct, currentSale, stockCover, stock, returnRate){
  if(returnRate>=15 && currentSale>0) return {label:'Fix Returns',key:'critical'};
  if(currentSale>0 && (stock<=0 || (Number.isFinite(stockCover)&&stockCover<14)) && (growthPct>=0 || score>=55)) return {label:'Replenish & Push',key:'high'};
  if(score>=75) return {label:'Push Now',key:'good'};
  if(score>=55) return {label:'Maintain',key:'blue'};
  if(score>=35) return {label:'Watch',key:'medium'};
  return {label:'Pause / Fix',key:'critical'};
}
function _buildOpportunityRows(){
  const win=Math.max(1,parseInt(document.getElementById('oppWindow')?.value||'30'));
  _oppRows=(master||[]).map(it=>{
    ...
    else if(current>0) growthPct=100;
    const rawGrowth=current<=0?0:(previous<=0?Math.min(40,20+current*4):_oppClamp(20+growthPct*0.20,0,40));
    const salesScore=_oppClamp(rawGrowth,0,40);
    const stock=Math.max(0,_opsNum(it.inv_stock)), wip=Math.max(0,_opsNum(it.inv_wip)), available=stock+wip;
    const drr=current/win;
    const stockCover=drr>0?stock/drr:null;
    const totalCover=drr>0?available/drr:null;
    const stockScore=current>0?_oppClamp((available/current)*30,0,30):0;
    const returnDen=current+st.currentRet;
    const returnRate=returnDen>0?(st.currentRet/returnDen*100):0;
...
function _opportunityFiltered(){
  if(!_oppRows.length) _buildOpportunityRows();
  const q=String(document.getElementById('oppSearch')?.value||'').trim().toLowerCase();
  const group=document.getElementById('oppGroup')?.value||'All';
  const taxon=document.getElementById('oppTaxon')?.value||'All';
...
function renderOpportunityScore(){
  _oppRows=[]; _buildOpportunityRows();
  const rows=_opportunityFiltered(), sum=document.getElementById('oppSummary'), host=document.getElementById('oppContent'); if(!host)return;
  const push=rows.filter(r=>r.action.label==='Push Now'||r.action.label==='Replenish & Push').length;
  const highGrowth=rows.filter(r=>(r.previousSale<=0&&r.currentSale>0)||r.growthPct>=20).length;
...
function _anomWindowStats(it, win){
  ...
  for(const e of (it.sales_entries||[])){
    ...
    else if(age<win*2){ previousQty+=Math.max(0,q); previousRet+=ret; previousRev+=rev; previousEntries.push(e); }
  }
  const currentGross=currentQty+currentRet, previousGross=previousQty+previousRet;
  return {
    currentQty,previousQty,currentRet,previousRet,currentRev,previousRev,currentEntries,previousEntries,
    currentReturnRate:currentGross>0?currentRet/currentGross*100:0,
...
function _buildSalesAnomalyRows(){
  ...
  for(const it of (master||[])){
    ...
    if(st.previousQty>=baseline && st.currentQty<=st.previousQty*0.5){
      ...
      const severity=(st.currentQty<=0||decline>=80)?'critical':decline>=65?'high':'medium';
      _anomAdd(rows,base,'SALE_DROP','Sudden Sale Drop',severity,`${decline.toFixed(1)}% drop`,`Current ${win}D sale ${Math.round(st.currentQty)} versus previous ${win}D sale ${Math.round(st.previousQty)}.`);
    }

    const spikeDelta=st.currentQty-st.previousQty;
    const spikeRatio=st.previousQty>0?st.currentQty/st.previousQty:null;
    const isSpike=(st.previousQty>0 && st.currentQty>=Math.max(baseline,st.previousQty*2) && spikeDelta>=baseline) || (st.previousQty<=0 && st.currentQty>=baseline*2);
    if(isSpike){
      const severity=(spikeRatio===null?st.currentQty>=baseline*4:spikeRatio>=3)?'high':'medium';
      const metric=spikeRatio===null?'New spike':`${spikeRatio.toFixed(1)}x sale`;
      _anomAdd(rows,base,'SALE_SPIKE','Unusual Sale Spike',severity,metric,`Current ${win}D sale ${Math.round(st.currentQty)} versus previous ${win}D sale ${Math.round(st.previousQty)}; increase ${Math.round(spikeDelta)} units.`);
    }

    const returnRise=st.currentReturnRate-st.previousReturnRate;
    const currentGross=st.currentQty+st.currentRet;
    if(currentGross>=baseline && st.currentReturnRate>=10 && (returnRise>=5 || (st.previousReturnRate>0 && st.currentReturnRate>=st.previousReturnRate*1.5))){
      const severity=st.currentReturnRate>=20?'critical':st.currentReturnRate>=15?'high':'medium';
      _anomAdd(rows,base,'RETURN_INCREASE','Unexpected Return Increase',severity,`${st.currentReturnRate.toFixed(1)}% return`,`Current return rate ${st.currentReturnRate.toFixed(1)}% versus previous ${st.previousReturnRate.toFixed(1)}% (${returnRise>=0?'+':''}${returnRise.toFixed(1)} percentage points).`);
    }

    const receiptAge=base.latestReceipt?_anomDateAge(base.latestReceipt):null;
    if(base.latestReceipt && receiptAge!==null && receiptAge<win && st.currentQty<=0 && (stock>0||wip>0)){
      const severity=stock>=50?'high':'medium';
      _anomAdd(rows,base,'STOCK_NO_SALE','Stock Movement Without Sale',severity,`${receiptAge}D since receipt`,`Latest receiving date ${base.latestReceipt}; no sale in latest ${win} days; Inv Stock ${Math.round(stock)}, Inv WIP ${Math.round(wip)}.`);
    }

    if(String(LOGIN_ROLE||'').toLowerCase()==='admin'){
      let negCount=0, negTotal=0, zeroRevenueQty=0, aboveMrpCount=0, orphanRevenue=0;
      const mrp=Math.max(0,_opsNum(it.mrp));
      for(const e of st.currentEntries){
        const q=_opsNum(e.qty), ret=Math.max(0,_opsNum(e.ret)), rev=_opsNum(e.rev);
        if(rev<0){ negCount++; negTotal+=rev; }
        if(q>0 && rev<=0) zeroRevenueQty+=q;
        if(q>0 && rev>0 && mrp>0 && rev/q>mrp*1.25) aboveMrpCount++;
        if(Math.abs(q)<0.0001 && ret<=0 && Math.abs(rev)>0.01) orphanRevenue+=rev;
      }
      const reasons=[];
      if(negCount) reasons.push(`${negCount} negative-revenue line(s), total ${fmt(negTotal)}`);
      if(zeroRevenueQty>0) reasons.push(`${Math.round(zeroRevenueQty)} sold unit(s) with zero/negative revenue`);
      if(aboveMrpCount) reasons.push(`${aboveMrpCount} line(s) above 125% of MRP per unit`);
      if(Math.abs(orphanRevenue)>0.01) reasons.push(`revenue ${fmt(orphanRevenue)} with zero sold and zero return quantity`);
      if(st.currentRev<0 && !negCount) reasons.push(`negative window revenue ${fmt(st.currentRev)}`);
      if(reasons.length){
        const severity=(negCount||st.currentRev<0)?'critical':zeroRevenueQty>0?'high':'medium';
        _anomAdd(rows,base,'REVENUE_ISSUE','Negative / Incorrect Revenue',severity,negCount?`${negCount} negative line(s)`:`${reasons.length} issue(s)`,reasons.join('; ')+'.');
      }
    }
  }
  _anomRows=rows.sort((a,b)=>_anomSeverityRank(a.severity)-_anomSeverityRank(b.severity)||a.typeLabel.localeCompare(b.typeLabel)||b.currentSale-a.currentSale||a.sku.localeCompare(b.sku));
  _opsFillTaxon('anomTaxon',_anomRows,r=>r.taxon);
  return _anomRows;
}
function _salesAnomalyFiltered(){
  if(!_anomRows.length) _buildSalesAnomalyRows();
  const type=document.getElementById('anomType')?.value||'All';
  const group=document.getElementById('anomGroup')?.value||'All';
...
function loadSalesAnomalies(fresh){
  ...
  return _loadOpsSupport(!!fresh).catch(()=>_opsSupport).then(()=>{_anomRows=[];_buildSalesAnomalyRows();renderSalesAnomalies();});
}
function renderSalesAnomalies(){
  _anomRows=[];_buildSalesAnomalyRows();
  const rows=_salesAnomalyFiltered(), sum=document.getElementById('anomSummary'), host=document.getElementById('anomContent'); if(!host)return;
  const affected=new Set(rows.map(r=>r.sku));
...
let _payTagRowsExport = [];       // current filtered tag summary rows, for export
...
function renderPayments(){
  ...
  if (agHost){
    ...
    _payAgingTotalsExport = AG_LABELS.map(k => ({bucket:k, amount: Math.round(agTotals[k])}));
    _payAgingTotalsExport.push({bucket:'Total', amount: Math.round(grand)});
  ...
  if (tagHost){
    ...
    const tbody = trows.map(s => {
      ...
      _payTagRowsExport.push({tag: s.tag, customers: s.customers, due: Math.round(s.due), overdue: Math.round(s.overdue), collected: Math.round(s.collected_month), balance: Math.round(s.balance)});
      return `<tr>
    ...
    _payTagRowsExport.push({tag:'Total', customers: tCust, due: Math.round(tDue), overdue: Math.round(tOv), collected: Math.round(tCol), balance: Math.round(tBal)});
...
/* ── Payments Received log (latest → oldest) ── */
function _payLogFiltered(){
  const d = _payData; if (!d) return [];
  const tag = (document.getElementById('payTag')?.value || '').toLowerCase();
  const q = (document.getElementById('paySearch')?.value || '').trim().toLowerCase();
  return (d.payments_log || []).filter(p => {
    if (tag && (p.tag||'').toLowerCase() !== tag) return false;
...
function renderPaymentsLog(){
  ...
  const body = rows.map(r => `<tr>
      ...
      <td style="padding:5px 8px;text-align:right;font-weight:700;color:#1f7a3a">${fmt(r.amount)}</td>
...
function exportPaymentsLog(){
  const rows = _payLogFiltered();
  if (!rows.length){ alert('No payments to export'); return; }
  const headers = ['Date','Customer Name','Tag','Amount','Vch No','Particulars'];
  const data = rows.map(r => [r.date||'', r.customer||'', r.tag||'', r.amount||0, r.vch_no||'', r.particulars||'']);
  _dlCsv(headers, data, 'payments_received_log');
}
window.renderPaymentsLog = renderPaymentsLog; window.exportPaymentsLog = exportPaymentsLog;

/* ── Per-table exports (respect current Tag/Search/Show filters) ── */
function exportPayToday(){
  const rows = _applyPaySort(_payFiltered());
  if (!rows.length){ alert('No rows to export'); return; }
  const headers = ['Customer Name','Tag','Term (days)','Due','Overdue','Balance'];
  const data = rows.map(r => [r.customer, r.tag||'', r.term_days||0, r.due||0, r.overdue||0, r.balance||0]);
  _dlCsv(headers, data, 'outstanding_till_today');
}
function exportPayMonthEnd(){
  const rows = _applyPaySort(_payFiltered());
  if (!rows.length){ alert('No rows to export'); return; }
  const headers = ['Customer Name','Tag','Due (till month-end)','Overdue (till month-end)','Balance'];
  const data = rows.map(r => [r.customer, r.tag||'', r.due_me||0, r.overdue_me||0, r.balance||0]);
  _dlCsv(headers, data, 'outstanding_till_month_end');
}
function exportPayAging(){
  if (!_payAgingTotalsExport.length){ alert('No data to export'); return; }
  const headers = ['Aging Bucket','Sum of Balance'];
  const data = _payAgingTotalsExport.map(a => [a.bucket, a.amount]);
...
function exportPayTagSummary(){
  ...
  const data = _payTagRowsExport.map(s => [s.tag, s.customers, s.due, s.overdue, s.collected, s.balance]);
  _dlCsv(headers, data, 'tag_wise_summary');
}
window.exportPayToday = exportPayToday; window.exportPayMonthEnd = exportPayMonthEnd;
window.exportPayAging = exportPayAging; window.exportPayWeek = exportPayWeek;
window.exportPayTagSummary = exportPayTagSummary;
...
showTab = function(t){
  ...
  const map = {
    ...
    concentration: {id: 'vConcentration', btn: 'm28'},
    demandpatterns: {id: 'vDemandPatterns', btn: 'm29'},
    ooslost: {id: 'vOosLost', btn: 'm30'},
  ...
  if (t === 'concentration') setTimeout(()=>{ try{ loadConcentrationRisk(); }catch(e){console.error(e);} }, 0);
...
mkCard = function(item, rev, conf, slow){
  ...
  return `<div class="card">
    ...
    <div class="cb">
      <div class="sku" style="cursor:pointer" onclick="openSkuDetails('${String(item.sku).replace(/'/g, "\\\\'")}')" title="SKU: ${escHtml(item.sku)} — view full details">${code}</div>
      <div class="card-meta">
...
/* ===== BUSINESS INTELLIGENCE: CONCENTRATION, DEMAND PATTERNS, OOS LOSS ===== */
...
function _bizShift(iso, days){
  const d = _bizDate(iso); if(!d) return '';
  d.setUTCDate(d.getUTCDate() + Number(days || 0));
  return d.toISOString().slice(0,10);
}
function _bizDaysInclusive(a,b){
  const d1=_bizDate(a), d2=_bizDate(b); if(!d1||!d2||d2<d1) return 0;
  return Math.floor((d2-d1)/86400000)+1;
}
function _bizPct(part,total){ return total>0 ? (part/total*100) : 0; }
function _bizPctText(v){ return Number.isFinite(v) ? v.toFixed(1)+'%' : '—'; }
function _bizMoney(v){ return '₹'+Math.round(Number(v)||0).toLocaleString('en-IN'); }
function _bizNum(v,d=0){ return (Number(v)||0).toLocaleString('en-IN',{minimumFractionDigits:d,maximumFractionDigits:d}); }
function _bizBar(pct){
  const w=Math.max(0,Math.min(100,Number(pct)||0));
  return `<div style="height:7px;background:#efe8d8;border-radius:999px;overflow:hidden;min-width:90px"><div style="height:100%;width:${w}%;background:linear-gradient(90deg,#b8960c,#d4af37);border-radius:999px"></div></div>`;
}
function _bizSetSelect(id, values, allLabel){
  const el=document.getElementById(id); if(!el)return;
  const cur=el.value||'All';
  const vals=Array.from(new Set((values||[]).map(v=>String(v||'').trim()).filter(Boolean))).sort((a,b)=>a.localeCompare(b,undefined,{sensitivity:'base'}));
  el.innerHTML=`<option value="All">${escHtml(allLabel||'All')}</option>`+vals.map(v=>`<option value="${escHtml(v)}">${escHtml(v)}</option>`).join('');
  el.value=vals.includes(cur)?cur:'All';
}
function _bizInitFilters(prefix){
  _bizSetSelect(prefix+'Taxon', allTaxons, 'All Taxons');
  _bizSetSelect(prefix+'Type', allTypes, 'All Types');
  _bizSetSelect(prefix+'Channel', allChannels, 'All Channels');
}
function _bizInitDateRange(id1,id2,days){
  const e1=document.getElementById(id1), e2=document.getElementById(id2); if(!e1||!e2)return;
  const end=_bizIso(todayISO)||new Date().toISOString().slice(0,10);
  if(!e2.value)e2.value=end;
  if(!e1.value)e1.value=_bizShift(end,-Math.max(1,Number(days)||180)+1);
}
function _bizEvents(){
  if(_bizEventCache) return _bizEventCache;
  const out=[];
  (master||[]).forEach(it=>{
    const sku=String(it.sku||'').trim(); if(!sku)return;
    const base={
      item:it, sku, skuName:String(it.sku_name||''), taxon:String(it.taxon||'General')||'General',
...
function _bizBaseFilter(prefix, includeDates=true){
  const group=document.getElementById(prefix+'Group')?.value||'All';
  const taxon=document.getElementById(prefix+'Taxon')?.value||'All';
  const type=document.getElementById(prefix+'Type')?.value||'All';
  const channel=document.getElementById(prefix+'Channel')?.value||'All';
  const search=String(document.getElementById(prefix+'Search')?.value||'').trim().toLowerCase();
  const d1=includeDates?_bizIso(document.getElementById(prefix+'D1')?.value):'';
  const d2=includeDates?_bizIso(document.getElementById(prefix+'D2')?.value):'';
  return e=>{
    if(group!=='All'&&e.group!==group)return false;
    if(taxon!=='All'&&e.taxon!==taxon)return false;
    if(type!=='All'&&e.type!==type)return false;
    if(channel!=='All'&&e.channel!==channel)return false;
    if(search&&!`${e.sku} ${e.skuName}`.toLowerCase().includes(search))return false;
    if(d1&&e.date<d1)return false;
    if(d2&&e.date>d2)return false;
    return true;
  };
}
function _bizRankMap(map,total){
  return Array.from(map.entries()).map(([name,v])=>({name,...v,share:_bizPct(v.rev,total)})).sort((a,b)=>b.rev-a.rev||b.qty-a.qty||a.name.localeCompare(b.name));
}
...
function _concCityRanks(filterMeta,totalRevenue){
  const city=new Map();
  const d1=_bizIso(document.getElementById('concD1')?.value), d2=_bizIso(document.getElementById('concD2')?.value);
  const type=document.getElementById('concType')?.value||'All';
  const channel=document.getElementById('concChannel')?.value||'All';
  const cityAllowed=(type==='All'||/website|online|d2c|direct/i.test(type))&&(channel==='All'||/d2c|website|online|direct/i.test(channel));
  if(!cityAllowed)return {rows:[],total:0,coverage:0};
  (master||[]).forEach(it=>{
    ...
    const webQty=webSales.reduce((s,e)=>s+Math.max(0,_opsNum(e.qty)),0);
    const webRev=webSales.reduce((s,e)=>s+Math.max(0,_opsNum(e.rev)),0);
    const fallbackSp=webQty>0&&webRev>0?webRev/webQty:Math.max(0,_opsNum(it.website_selling_price)||_opsNum(it.avg_selling_price)||_opsNum(it.mrp));
    (it.website_city_events||[]).forEach(ev=>{
      ...
      const x=city.get(name)||{rev:0,qty:0}; x.rev+=rev; x.qty+=qty; city.set(name,x);
    });
  });
  const total=Array.from(city.values()).reduce((s,v)=>s+v.rev,0);
...
function _dpDayIndex(iso){const d=_bizDate(iso);return d?(d.getUTCDay()+6)%7:null;}
function _dpCalendarCounts(d1,d2){
  const days=Array(7).fill(0); let weekday=0,weekend=0,start=0,end=0,pay=0,nonpay=0;
  const a=_bizDate(d1),b=_bizDate(d2); if(!a||!b||b<a)return {days,weekday,weekend,start,end,pay,nonpay};
  for(let d=new Date(a);d<=b;d.setUTCDate(d.getUTCDate()+1)){
    const idx=(d.getUTCDay()+6)%7;days[idx]++; if(idx>=5)weekend++;else weekday++;
    const dom=d.getUTCDate(); if(dom<=10)start++; if(dom>=21)end++; if(dom<=5||dom>=25)pay++;else nonpay++;
  }
  return {days,weekday,weekend,start,end,pay,nonpay};
}
...
function renderDemandPatterns(){
  ...
  events.forEach(e=>{
    const idx=_dpDayIndex(e.date);if(idx===null)return;const v=value(e);dayAgg[idx].qty+=Math.max(0,e.qty);dayAgg[idx].rev+=Math.max(0,e.rev);dayAgg[idx].lines++;
    if(idx>=5)weekendValue+=v;else weekdayValue+=v;
    const dom=Number(e.date.slice(8,10));if(dom<=10)startValue+=v;if(dom>=21)endValue+=v;if(dom<=5||dom>=25)payValue+=v;else nonpayValue+=v;
    const s=skuMap.get(e.sku)||{item:e.item,totalQty:0,totalRev:0,days:Array(7).fill(0),weekend:0,weekday:0,pay:0,nonpay:0};
    s.totalQty+=Math.max(0,e.qty);s.totalRev+=Math.max(0,e.rev);s.days[idx]+=Math.max(0,e.qty);if(idx>=5)s.weekend+=Math.max(0,e.qty);else s.weekday+=Math.max(0,e.qty);if(dom<=5||dom>=25)s.pay+=Math.max(0,e.qty);else s.nonpay+=Math.max(0,e.qty);skuMap.set(e.sku,s);
  });
  const averages=dayAgg.map((r,i)=>({...r,avg:(metricMoney?r.rev:r.qty)/Math.max(1,r.calendarDays)}));
  const best=averages.slice().sort((a,b)=>b.avg-a.avg)[0]; const maxAvg=Math.max(0,...averages.map(r=>r.avg));
  const weekdayAvg=weekdayValue/Math.max(1,cal.weekday),weekendAvg=weekendValue/Math.max(1,cal.weekend),weekendUplift=_dpUplift(weekendAvg,weekdayAvg);
  const startAvg=startValue/Math.max(1,cal.start),endAvg=endValue/Math.max(1,cal.end),endUplift=_dpUplift(endAvg,startAvg);
  const payAvg=payValue/Math.max(1,cal.pay),nonpayAvg=nonpayValue/Math.max(1,cal.nonpay),payUplift=_dpUplift(payAvg,nonpayAvg);
  const fd1=_bizIso(document.getElementById('dpFestD1')?.value),fd2=_bizIso(document.getElementById('dpFestD2')?.value);let festUplift=null,festAvg=0,prevFestAvg=0,festDays=0;
  if(fd1&&fd2&&(festDays=_bizDaysInclusive(fd1,fd2))>0){
    const prevEnd=_bizShift(fd1,-1),prevStart=_bizShift(fd1,-festDays);const all=_dpBaseEvents(false);
    const fVal=all.filter(e=>e.date>=fd1&&e.date<=fd2).reduce((s,e)=>s+value(e),0);const pVal=all.filter(e=>e.date>=prevStart&&e.date<=prevEnd).reduce((s,e)=>s+value(e),0);
    festAvg=fVal/festDays;prevFestAvg=pVal/festDays;festUplift=_dpUplift(festAvg,prevFestAvg);
  }
  const fmtMetric=v=>metricMoney?_bizMoney(v):_bizNum(v,2);
  if(sum)sum.innerHTML=
    _opsKpi('Best Sale Day',best?best.name:'—',best?`${fmtMetric(best.avg)} average/day`:'No data')+
    _opsKpi('Weekend vs Weekday',_dpUpliftText(weekendUplift),`${fmtMetric(weekendAvg)} vs ${fmtMetric(weekdayAvg)} avg/day`)+
    _opsKpi('Month-End Uplift',_dpUpliftText(endUplift),`${fmtMetric(endAvg)} vs ${fmtMetric(startAvg)} avg/day`)+
    _opsKpi('Payday Impact',_dpUpliftText(payUplift),`${fmtMetric(payAvg)} vs ${fmtMetric(nonpayAvg)} avg/day`)+
    _opsKpi('Festival Uplift',festDays?_dpUpliftText(festUplift):'Select dates',festDays?`${fmtMetric(festAvg)} vs ${fmtMetric(prevFestAvg)} avg/day`:'Custom festival period')+
    _opsKpi('Analysed Demand',fmtMetric(events.reduce((s,e)=>s+value(e),0)),`${_bizDaysInclusive(d1,d2)} calendar days`);
...
def api_ops_support():
    """Small support payload for Smart Alerts and Inventory Ageing.

    Production rows are aggregated server-side from the full PPC-WIP cache so
    the client is not limited by the Production tab's 1,000-row display cap.
    """
    if session.get("role") not in ("admin", "employee"):
        return jsonify({"error": "login required"}), 401
    try:
        ...
        if fresh:
            ...
            _PROD_CACHE["ts"] = 0
            _TARGET_CACHE["rows"] = None
            _TARGET_CACHE["ts"] = 0

        # Populates _PROD_CACHE with all source rows. Returned rows may be capped,
        # therefore we intentionally read the underlying cache afterwards.
        _build_production()
        rows_all = list(_PROD_CACHE.get("rows") or [])
        ...
        for r in rows_all:
            ...
            recv_raw = r.get("receiving_date") or ""
            recv_dt = parse_date_any(recv_raw) if recv_raw else None
            if recv_dt is not None:
                recv_iso = recv_dt.strftime("%Y-%m-%d")
                if recv_iso > latest_receipts.get(sku, ""):
                    latest_receipts[sku] = recv_iso

            bal = float(r.get("bal_qty") or 0)
            if bal <= 0:
                continue
            p = pending.setdefault(sku, {
                "sku": sku, "balance_qty": 0.0, "order_qty": 0.0,
                "recv_qty": 0.0, "oldest_order_date": "",
                "latest_delivery_date": "", "order_nos": [],
                "channels": [], "order_types": [],
            })
            p["balance_qty"] += bal
            p["order_qty"] += float(r.get("order_qty") or 0)
            p["recv_qty"] += float(r.get("recv_qty") or 0)
            od = str(r.get("date") or "")
            if od and (not p["oldest_order_date"] or od < p["oldest_order_date"]):
                p["oldest_order_date"] = od
            dd = str(r.get("delivery_iso") or "")
            if dd and dd > p["latest_delivery_date"]:
                p["latest_delivery_date"] = dd
            ono = str(r.get("order_no") or "").strip()
            if ono and ono not in p["order_nos"]:
                p["order_nos"].append(ono)
            ch = str(r.get("channel") or "").strip()
            if ch and ch not in p["channels"]:
                p["channels"].append(ch)
            typ = str(r.get("order_type") or "").strip()
            if typ and typ not in p["order_types"]:
                p["order_types"].append(typ)

        target = _build_target_report()
        return jsonify({
            "today": now_ist().strftime("%Y-%m-%d"),
            "production_pending": list(pending.values()),
            "latest_receipts": [
                {"sku": sku, "latest_receiving_date": dt}
                for sku, dt in sorted(latest_receipts.items())
            ],
            "target_rows": target.get("rows", []),
            "target_totals": target.get("totals", {}),
            "target_month": target.get("month_selected", ""),
        })
    except Exception as e:
...
def _build_sku_costs():
    ...
    try:
        ...
        for it in comp:
            ...
            out.append({
                "sku":         sk,
                "sku_name":    it.get("sku_name") or cn_sku_label(sk),
...
def _build_payments():
    ...
    for nm, entries in by_cust.items():
        ...
        cust_carry_over = 0.0   # balance jo is month se PEHLE hi due/overdue tha aur abhi tak nahi mila
        ...
        # tha aur customer ne abhi tak nahi diya — vo is month ke Week 1 me "Overdue
        # Becoming Due" ke roop me dikhna chahiye (amount = wahi jo Outstanding table
        # ke "Balance" column me hai). month_start_g ki date daal rahe hain taaki ye
        # week-wise bucketing (ws <= date <= we) me apne aap Week 1 me chala jaye.
        if cust_carry_over > 0.009:
            _cust_week_due.append([month_start_g.strftime("%Y-%m-%d"), round(cust_carry_over, 2)])
        ...
        carry_over_total += cust_carry_over
    ...
    for due_date, rem in all_due_pairs:
        ...
        for wi, (ws_, we_) in enumerate(week_bounds):
            if ws_ <= due_date <= we_:
                ...
                break
    # CARRY-OVER: pichle mahine (ya usse pehle) se jo due/overdue tha aur abhi tak
    # nahi mila, uska poora balance is month ke Week 1 me "Overdue Becoming Due"
    # me add hota hai (company-wide / unfiltered view ke liye — filtered view
    # frontend me row.week_due se hi recompute hota hai, jisme carry-over already
    # month_start ki date se Week 1 me daala gaya hai).
    if week_overdue_amt:
